...
public class Realm {
    ...
    private static void command(String string) throws IOException {
        ...
        if (player == null) {
            ...
            System.out.println(String.format("Приветствую %s! Удачи!", player.getName()));
        ...
        switch (string) {
            case "1": {
                ...
                int healedAmount = merchant.buyPotion(player);
                if (healedAmount > 0) {
                    System.out.println("Вы успешно купили зелье! Здоровье восстановлено на " + healedAmount + " единиц. Теперь у вас " + player.getHealthPoints() + " здоровья.");
                }
                printNavigation();
                command(br.readLine());
            }
    ...
    private static void commitFight() {
        battleScene.fight(player, createMonster(), new FightCallback() {
            ...
            public void fightWin() {
                ...
                System.out.println("Желаете продолжить поход или вернуться в город? (продолжить/в город)");
                try {
                    command(br.readLine());
                } catch (IOException e) {
                    e.printStackTrace();
